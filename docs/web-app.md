# dbxclean, the web app

The first version of this idea, and still in the repository: a FastAPI
backend and a React frontend that work against the Dropbox API rather than a
local directory. It does things the terminal tool does not, namely perceptual
hashing for similar images, image quality scoring and renaming suggestions,
and it needs a Dropbox access token, a database and two processes to run.

The terminal tool in [the README](../README.md) came second. It has no
dependencies, no token and no server, and it never deletes anything.
...
It leans on the Dropbox API for cloud file access and on imagehash for the
perceptual hashing.
